...
CREATE PROCEDURE InsertarProducto(
    ...
    IN p_talla VARCHAR(5),
    IN p_precio DECIMAL(10,2),
    IN p_stock INT
)
BEGIN
    INSERT INTO Producto (nombre, talla, precio, stock) 
    VALUES (p_nombre, p_talla, p_precio, p_stock);
END;
//
DELIMITER ;

SELECT p.nombre, p.talla, p.precio, pp.cantidad
FROM Producto p
...
UPDATE Producto SET precio = precio * 1.20 WHERE producto_id = 1;
UPDATE Producto SET stock = stock + 10 WHERE producto_id = 2;
UPDATE Empleado SET cargo = 'Supervisor de Ventas' WHERE empleado_id = 1;
